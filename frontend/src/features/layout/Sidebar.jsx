...
function Sidebar({ abierto = false, onCerrar }) {
    ...
    return (
        <aside
            className={`fixed left-0 top-0 z-50 flex h-screen w-60 flex-col border-r border-slate-800/70 bg-slate-950 px-3 py-5 text-white shadow-2xl transition-transform duration-300 ease-in-out lg:translate-x-0 lg:shadow-none ${
        ...
        >
            ...
            <nav className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pr-1 [scrollbar-width:thin] [scrollbar-color:#475569_transparent]">
                {navPrincipal.map(({ nombre, ruta, icono: Icono }) => (
                    <NavLink
                        ...
                        className={({ isActive }) =>
                            `group flex items-center gap-3 rounded-xl px-2.5 py-2 text-xs font-semibold transition ${
                                isActive
